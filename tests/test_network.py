...
@pytest.mark.e2e
async def test_connectivity_job_with_http_port(
    secret_job: Dict[str, Any], helper: Helper
) -> None:
...
@pytest.mark.e2e
def xtest_connectivity_job_without_http_port(secret_job, helper):
...
@pytest.mark.e2e
def xtest_check_isolation(secret_job, helper_alt):
    http_job = secret_job(True)
